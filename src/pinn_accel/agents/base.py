...
class BaseWeightAgent:
    ...
    def state_dim(self) -> int:
        return 3 * len(self.component_names) + 1
    ...
    def make_state(
        ...
        if self.prev_losses is None:
            trends = np.zeros_like(losses_np, dtype=np.float32)
        else:
            trends = np.log((self.prev_losses + eps) / (losses_np + eps)).astype(np.float32)
        ...
        return np.concatenate(
            [
                ...
                trends,
                normalized_weights,
                np.array([np.clip(progress, 0.0, 1.0)], dtype=np.float32),
            ]
        )
    ...
    def _project_weights(self, weights: np.ndarray) -> np.ndarray:
        ...
        upper = float("inf") if self.max_weight_share is None else target * self.max_weight_share
        ...
        for _ in range(32):
            ...
            if delta > 0:
                ...
                eligible = np.ones_like(projected, dtype=bool) if np.isinf(upper) else slack > 1e-8
                allocation = np.maximum(projected[eligible], 1.0) if np.isinf(upper) else slack[eligible]
                ...
                projected[eligible] += delta * removable[eligible] / float(np.sum(removable[eligible]))
